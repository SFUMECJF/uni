...
void serve_web_page(char *pathname, FILE *client_stream) {
    // Attempts to access file with pathname
    if (access(pathname, F_OK ) != -1 ) {
        //Access successful
        //Printing 200 header
        fprintf(client_stream, HEADER_200);
        
        //Opening existing file in read and coping over to client_stream
        FILE* requested;
        requested = fopen(pathname, "r");
        int character;
        while ((character = fgetc(requested) ) != EOF) {
            fputc(character, client_stream);
        }
        fclose(requested);
    } else {
        //Access unsuccessful
