...
invalid:
    ...
    scanf("%d", &truth2);
    if (truth2 == 1 && truth && total >= SECRET_NUMBER){
    printf("Your roll was larger than the cutoff. Congratulations, you have made it to the next stage\n");
    }
    else if (truth2 == 1 && total<= SECRET_NUMBER){
    printf("Your roll was under the cuttoff. Sorry, but you have failed the course. We advise you to attempt the course again next teaching term. \n");
    }
    else if (truth2 == 0){
    goto roll;
    }
    //Goes back to input of user results if user has typed their results wrong
    else {
    goto invalid;
    }
    }
