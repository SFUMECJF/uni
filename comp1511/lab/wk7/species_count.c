...
void species_count(char species[], int n_sightings, struct pod sightings[n_sightings], int *n_pods, int *n_whales) {
    ...
    while (counter < n_sightings) {
        compare = strcmp(sightings[counter].species, species);
        if (compare == 0) {
            total += sightings[counter].how_many;
            sight += 1;
        }
        counter++;
        compare = 1;
    ...
    *n_pods = sight; // CHANGE ME
    *n_whales = total; // CHANGE ME
}
