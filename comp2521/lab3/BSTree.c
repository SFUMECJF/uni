...
void BSTreeLevelOrder (BSTree t)
{
	if (t == NULL) {
		...
		//Initialise queue
		struct QueueRep *Queue = newQueue();
		//Adds root node to queue
		QueueJoin(Queue, t);
		//Initialises copy of new head
		struct BSTNode *head;

		//Loop runs as long as there is something in queue
		//Will definitely print out everything if algorithm given is followed
		while(!QueueIsEmpty(Queue)) {
			//Takes head of queue
			head = QueueLeave(Queue);
			//Prints value in node
			printf("%d ", head->value);
			//Add left/right child if there is any
			if (head->left != NULL) {
				QueueJoin(Queue, head->left);
			}
			if (head->right != NULL) {
				QueueJoin(Queue, head->right );
			}
		}
		dropQueue(Queue);
	}
...
{
	//Returns nothing if it is null
	if (t == NULL) {
		return 0;
	//Cycles through and adds the sum off all leaves one by one going to each node
	} else {
