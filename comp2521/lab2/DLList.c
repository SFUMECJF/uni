...
void DLListBefore (DLList L, char *it)
{
	...
	if (L->curr != L->first) {
		struct DLListNode *copyprev = L->curr->prev;
		copyprev->next = new;
		new->next = L->curr;
		new->prev = copyprev;
		//sets prev of new node
		L->curr->prev = new;
		L->curr = new;
	} else if (L->nitems == 0) {
		...
		//else sets new node as the "new first"
		L->first = new;
		L->first->next = L->curr;
		L->curr->prev = L->first;
		L->curr = L->first;
...
void DLListAfter (DLList L, char *it)
{
	...
	if (L->nitems == 0) {
		...
		struct DLListNode *copynext = L->curr->next;
		//Inserts new node after current. Also set new current node as new
		L->curr->next = new;
		new->next = copynext;
		//sets prev of new node
		new->next->prev = new;
...
{
	assert (L != NULL);
	//assert it is not empty list
	assert (L->curr != NULL);
	struct DLListNode *temp = L->curr;		

	if (L->curr != L->first && L->curr != L->last) {
		//deletes a node in the middle of the dlinked list.
		struct DLListNode *copynext = L->curr->next;
		struct DLListNode *copyprev = L->curr->prev;
		...
		//deletes and sets new first node
		L->first = L->first->next;
		L->curr = L->first;
		free(temp->value);
		free(temp);
	} else if (L->curr == L->last) {
		//deletes and sets new last node
		L->curr = L->curr->prev;
		L->last = L->curr;
		L->last->next = NULL;
		free(temp->value);
