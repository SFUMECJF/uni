...
void IntListInsertInOrder (IntList L, int v)
{
	...
	//creates copy of struct/new list to work with
	struct IntListNode *new = newIntListNode (v);
	
	//struct IntListrep *copy = L;
	//checks if current list is empty and attempt to cycle through if not
	//Inserts new node if the list is in fact empty.
	if (L->first == NULL)
	{
		L->first = L->last = new;
		L->first->next = L->last;
		L->last->next = NULL;
	//sets new first number if new is less than the first number
	} else if (new->data < L->first->data) 
	{
		new->next = L->first;
		L->first = new;
	//sets new last number if it is more than the last number
	...
	{
		struct IntListNode *compare = L->first;
		while (1) 
		{
			if (new->data <= compare->next->data) {
